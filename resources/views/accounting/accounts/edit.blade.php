...
@section('page_title', 'Edit Account: ' . $account->name)
...
@section('page_content')
    <form action="{{ route('accounting.accounts.update', $account) }}" method="POST">
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" name="name" class="form-control modern-input @error('name') is-invalid @enderror" value="{{ old('name', $account->name) }}" required>
                            @error('name')
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" name="code" class="form-control modern-input @error('code') is-invalid @enderror" value="{{ old('code', $account->code) }}" required>
                            @error('code')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select name="account_group_id" class="form-control select2 @error('account_group_id') is-invalid @enderror" required>
                                ...
                                @foreach($groups as $group)
                                    <option value="{{ $group->id }}" {{ old('account_group_id', $account->account_group_id) == $group->id ? 'selected' : '' }}>
                        ...
                        <div class="form-group">
                            ...
                            <select name="account_type" class="form-control modern-select @error('account_type') is-invalid @enderror" required>
                                @foreach($accountTypes as $key => $label)
                                    <option value="{{ $key }}" {{ old('account_type', $account->account_type) == $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                            @error('account_type')
                                <div class="invalid-feedback">{{ $message }}</div>
                ...
                <div class="row mt-3">
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="number" name="opening_balance" class="form-control modern-input @error('opening_balance') is-invalid @enderror" value="{{ old('opening_balance', $account->opening_balance) }}" step="0.01" min="0">
        ...
        <!-- Bank Details -->
        <div class="card modern-card mt-4" id="bank-details" style="{{ $account->account_type === 'bank' ? '' : 'display: none;' }}">
                ...
                <div class="row">
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <input type="text" name="bank_name" class="form-control modern-input" value="{{ old('bank_name', $account->bank_name) }}">
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="form-group">
                            <label class="form-label">Account Number</label>
                            <input type="text" name="bank_account_number" class="form-control modern-input" value="{{ old('bank_account_number', $account->bank_account_number) }}">
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="form-group">
                            <label class="form-label">IFSC/Routing Code</label>
                            <input type="text" name="ifsc_code" class="form-control modern-input" value="{{ old('ifsc_code', $account->ifsc_code) }}">
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            <textarea name="notes" class="form-control modern-textarea" rows="3">{{ old('notes', $account->notes) }}</textarea>
                        </div>
                    </div>
                </div>
                <div class="row mt-3">
                    <div class="col-md-12">
                        <div class="form-check">
                            <input type="checkbox" name="is_active" class="form-check-input" id="is_active" value="1" {{ old('is_active', $account->is_active) ? 'checked' : '' }}>
