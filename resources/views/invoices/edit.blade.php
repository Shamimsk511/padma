...
@section('header_actions')
    <div class="btn-group">
        <a href="{{ route('invoices.show', $invoice) }}" class="btn modern-btn modern-btn-info">
            <i class="fas fa-eye"></i> View
        </a>
        <a href="{{ route('invoices.print', $invoice) }}" target="_blank" class="btn modern-btn modern-btn-secondary">
...
@section('page_content')
            ...
            <div class="card-body">
                ...
                <div class="row mb-4">
                    <div class="col-md-6">
                        ...
                        <div class="toggle-group">
                            <input type="radio" id="invoice_type_tiles" name="invoice_type" value="tiles"
                                   {{ $invoice->invoice_type == 'tiles' ? 'checked' : '' }}>
                            ...
                            <input type="radio" id="invoice_type_other" name="invoice_type" value="other"
                                   {{ $invoice->invoice_type == 'other' ? 'checked' : '' }}>
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="toggle-group">
                            <input type="radio" id="delivery_pending" name="delivery_status" value="pending"
                                   {{ $invoice->delivery_status == 'pending' ? 'checked' : '' }}>
                            <label for="delivery_pending" class="toggle-option">
                                <i class="fas fa-clock"></i> Pending
                            </label>

                            <input type="radio" id="delivery_partial" name="delivery_status" value="partial"
                                   {{ $invoice->delivery_status == 'partial' ? 'checked' : '' }}>
                            <label for="delivery_partial" class="toggle-option">
                                <i class="fas fa-truck-loading"></i> Partial
                            </label>

                            <input type="radio" id="delivery_delivered" name="delivery_status" value="delivered"
                                   {{ $invoice->delivery_status == 'delivered' ? 'checked' : '' }}>
                            <label for="delivery_delivered" class="toggle-option">
                                <i class="fas fa-check"></i> Delivered
                            </label>
                        </div>
                        <div class="alert alert-warning mt-2" id="delivery-stock-warning" style="display:none;">
                        ...
                        <div class="info-section">
                            ...
                            <div class="section-content">
                                <div class="form-group">
                                    ...
                                    <input type="text" id="invoice_number_display" name="invoice_number" class="form-control modern-input"
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="date" name="invoice_date" id="invoice_date" class="form-control modern-input"
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            ...
                                            <input type="text" id="customer_balance" class="form-control modern-input"
                                                   value="{{ $customerLedgerBalance['formatted'] ?? number_format($invoice->customer->outstanding_balance ?? 0, 2) }}" readonly>
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <input type="text" id="customer_phone" name="customer_phone" class="form-control modern-input"
                                                   value="{{ $invoice->customer->phone ?? '' }}" readonly tabindex="-1">
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <label for="customer_address">Address</label>
                                            <input type="text" id="customer_address" name="customer_address" class="form-control modern-input"
                                                   value="{{ $invoice->customer->address ?? '' }}" readonly tabindex="-1">
                                        </div>
                                    </div>
                                </div>
                                            ...
                                            <div class="input-group">
                                                <select name="referrer_id" id="referrer_id" class="form-control select2 modern-select">
                                                    ...
                                                    @foreach($referrers as $referrer)
                                                        <option value="{{ $referrer->id }}" {{ old('referrer_id', $invoice->referrer_id) == $referrer->id ? 'selected' : '' }}>
                    ...
                    <table class="table modern-table compact-table" id="product-table">
                        ...
                        <tbody>
                            ...
                            <tr class="product-row"
                                data-box-pcs="{{ $item->product->category->box_pcs ?? 0 }}"
                                data-pieces-feet="{{ $item->product->category->pieces_feet ?? 0 }}"
                                data-weight-value="{{ $item->product->category->weight_value ?? 0 }}"
                                data-weight-unit="{{ $item->product->category->weight_unit ?? '' }}"
                                ...
                                <td>
                                    <select name="product_id[]" class="form-control select2 product-select modern-select" required tabindex="5">
                                        ...
                                        @foreach($products as $product)
                                            @php
                                                // For the currently selected product, add back the invoiced quantity to available stock
                                                $adjustedStock = $product->current_stock;
                                                if ($item->product_id == $product->id && $invoice->delivery_status == 'delivered') {
                                                    $adjustedStock += $item->quantity;
                                                }
                                            @endphp
                                            <option value="{{ $product->id }}"
                                                ...
                                                {{ $product->name }} ({{ $adjustedStock }})
                                            </option>
                                        @endforeach
                                        <option value="__new__">+ New Product</option>
                                    </select>
                                    <input type="hidden" class="product-company-hidden" value="{{ $item->product->company->name ?? '' }}">
                                    <input type="hidden" class="product-category-hidden" value="{{ $item->product->category->name ?? '' }}">
                                </td>
                                <td>
                                    <input type="text" name="description[]" class="form-control product-description modern-input compact-input"
                                           value="{{ $item->description }}" required placeholder="Desc">
                                    <input type="text" name="code[]" class="form-control product-code modern-input compact-input mt-1"
                                           value="{{ $item->code }}" placeholder="Code">
                                </td>
                                <td>
                                    <input type="number" name="quantity[]" class="form-control product-quantity modern-input compact-input"
                                           value="{{ $item->quantity }}" step="0.01" min="0.01" required placeholder="0">
                                </td>
                                <td class="box-pieces-cell">
                                    <input type="number" name="boxes[]" class="form-control product-boxes modern-input compact-input"
                                           value="{{ $item->boxes }}" readonly>
                                </td>
                                <td class="box-pieces-cell">
                                    <input type="number" name="pieces[]" class="form-control product-pieces modern-input compact-input"
                                           value="{{ $item->pieces }}" readonly>
                                </td>
                                <td>
                                    <input type="number" name="unit_price[]" class="form-control product-price modern-input compact-input"
                                           value="{{ $item->unit_price }}" step="0.01" min="0" required placeholder="0">
                                </td>
                                <td>
                                    <input type="number" name="item_total[]" class="form-control product-total modern-input compact-input"
                                           value="{{ $item->total }}" step="0.01" min="0" readonly required>
                                </td>
                                <td>
                    ...
                    <div class="card-body">
                        <div class="form-group">
                            ...
                            <select name="payment_method" id="payment_method" class="form-control modern-select" required>
                                <option value="cash" {{ $invoice->payment_method == 'cash' ? 'selected' : '' }} data-account-type="cash">Cash</option>
                                <option value="bank" {{ $invoice->payment_method == 'bank' ? 'selected' : '' }} data-account-type="bank">Bank Transfer</option>
                                <option value="mobile_bank" {{ $invoice->payment_method == 'mobile_bank' ? 'selected' : '' }} data-account-type="bank">Mobile Banking</option>
                                <option value="cheque" {{ $invoice->payment_method == 'cheque' ? 'selected' : '' }} data-account-type="bank">Cheque</option>
                            </select>
                        </div>
                        ...
                        @php
                            // Get existing account_id from the invoice's debit transaction (if any)
                            $existingAccountId = null;
                            $debitTransaction = $invoice->transactions()->where('type', 'debit')->first();
                            if ($debitTransaction && $debitTransaction->account_id) {
                                $existingAccountId = $debitTransaction->account_id;
                            }
                        @endphp
                        <div class="form-group">
                            ...
                            <select name="account_id" id="account_id" class="form-control modern-select">
                                ...
                                @foreach ($cashBankAccounts as $account)
                                    ...
                                    <option value="{{ $account->id }}"
                                        ...
                                        {{ $existingAccountId == $account->id ? 'selected' : '' }}>
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="notes" id="notes" class="form-control modern-textarea" rows="3" placeholder="Additional notes or comments...">{{ $invoice->notes }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="card-body">
                        <div class="totals-section">
                            <div class="total-row">
                                ...
                                <input type="number" name="subtotal" id="subtotal" class="form-control modern-input"
                                       value="{{ $invoice->subtotal }}" step="0.01" readonly>
                            </div>

                            <div class="total-row">
                                <label>Discount:</label>
                                <input type="number" name="discount" id="discount" class="form-control modern-input"
                                       value="{{ $invoice->discount }}" step="0.01" min="0">
                            </div>

                            <div class="total-row total-main">
                                <label>Total:</label>
                                <input type="number" name="total" id="total" class="form-control modern-input total-input"
                                       value="{{ $invoice->total }}" step="0.01" min="0" readonly required>
                            </div>

                            <div class="total-row">
                                <label>Paid Amount:</label>
                                <input type="number" name="paid_amount" id="paid_amount" class="form-control modern-input"
                                       value="{{ $invoice->paid_amount }}" step="0.01" min="0" required>
                            </div>

                            <div class="total-row">
                                <label>Due Amount:</label>
                                <div class="input-group">
                                    <input type="number" name="due_amount" id="due_amount" class="form-control modern-input"
                                           value="{{ $invoice->due_amount }}" step="0.01" readonly>
                                    <div class="input-group-append">
    ...
    <div class="text-center mt-4 mb-4">
        <button type="submit" class="btn modern-btn modern-btn-primary btn-lg" id="update-invoice-btn">
            <i class="fas fa-save"></i> Update Invoice
        </button>
        <a href="{{ route('invoices.show', $invoice) }}" class="btn modern-btn modern-btn-info btn-lg ml-2">
...
const originalDeliveryStatus = '{{ $invoice->delivery_status }}';
const existingAccountId = {{ $existingAccountId ?? 'null' }};
...
$(document).ready(function() {
    ...
    // Auto-select accounting account based on payment method (only if no existing account)
    $('#payment_method').on('change', function() {
        ...
        // If user has manually selected an account or there's an existing account, don't auto-change
        if (accountSelect.data('manual-selection') || existingAccountId) {
            return;
        }
    ...
    // Track manual account selection
    $('#account_id').on('change', function() {
        $(this).data('manual-selection', true);
    });

    // Only trigger auto-selection if no existing account
    ...
    // Show delivery warning if switching to delivered
    const currentDeliveryStatus = $('input[name="delivery_status"]:checked').val();
    if (currentDeliveryStatus === 'delivered') {
        toggleStockWarnings(true);
    }
});
...
function addProductRow() {
    ...
    newRow.removeData('original-qty');
    ...
    if (productId && productId !== '__new__') {
        $.ajax({
            ...
            success: function(data) {
                ...
                row.data('weight-value', data.category ? data.category.weight_value : 0);
                row.data('weight-unit', data.category ? data.category.weight_unit : '');
        ...
        row.find('.product-description, .product-company-hidden, .product-category-hidden, .product-price, .product-quantity, .product-boxes, .product-pieces, .product-total').val('');
        row.data('box-pcs', 0);
...
function submitInvoiceForm(formData, forceNegativeStock = false) {
    ...
    if (!hasValidProduct) {
        ...
        $('#update-invoice-btn').prop('disabled', false).html('<i class="fas fa-save"></i> Update Invoice');
        return;
    }
        ...
        success: function(response) {
            if (response.success) {
                Swal.fire({
                    icon: 'success',
                    title: 'Success!',
                    html: `Invoice updated successfully!<br><strong>Invoice Number: ${response.invoice_number || '{{ $invoice->invoice_number }}'}</strong>`,
                    showConfirmButton: false,
                    timer: 2000
                }).then(() => {
                    window.location.href = response.redirect || "{{ route('invoices.show', $invoice) }}";
                });
            } else {
                Swal.fire({
                    icon: 'success',
...
function showStockWarningAlert(stockIssues, formData) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            const submitBtn = $('#update-invoice-btn');
            submitBtn.prop('disabled', false).html('<i class="fas fa-save"></i> Update Invoice');
        }
    });
}
</script>
