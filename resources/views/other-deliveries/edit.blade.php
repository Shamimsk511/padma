...
@section('title', 'Edit Other Delivery')

@section('page_title', 'Edit Delivery Challan #' . $otherDelivery->challan_number)

@section('header_actions')
    <div class="header-actions-group">
        <a href="{{ route('other-deliveries.show', $otherDelivery) }}" class="btn modern-btn modern-btn-outline">
            <i class="fas fa-eye"></i> <span class="btn-text">View Delivery</span>
        </a>
...
@section('page_content')
    ...
    @if($otherDelivery->status === 'delivered')
        <div class="alert alert-warning alert-dismissible modern-alert">
            ...
            <strong>Note:</strong> This delivery has been marked as delivered. Some fields may be restricted for editing.
        </div>
    @endif

    <form action="{{ route('other-deliveries.update', $otherDelivery) }}" method="POST" id="delivery-form">
        @csrf
        @method('PUT')
                ...
                <div class="card modern-card">
                    ...
                    <div class="card-body modern-card-body">
                        <div class="form-group">
                            ...
                            <input type="text" name="challan_number" id="challan_number" class="form-control modern-input" value="{{ $otherDelivery->challan_number }}" readonly>
                            <small class="text-muted">Challan number cannot be changed after creation</small>
                        </div>
                        
                        <div class="form-group">
                            <label for="delivery_date" class="form-label">Delivery Date <span class="text-danger">*</span></label>
                            <input type="date" name="delivery_date" id="delivery_date" class="form-control modern-input" value="{{ $otherDelivery->delivery_date }}" required>
                        </div>

                        <div class="form-group">
                            <label for="status" class="form-label">Delivery Status <span class="text-danger">*</span></label>
                            <select name="status" id="status" class="form-control modern-select" required>
                                <option value="pending" {{ $otherDelivery->status === 'pending' ? 'selected' : '' }}>Pending</option>
                                <option value="in_transit" {{ $otherDelivery->status === 'in_transit' ? 'selected' : '' }}>In Transit</option>
                ...
                <div class="card modern-card">
                    ...
                    <div class="card-body modern-card-body">
                        <div class="form-group">
                            ...
                            <input type="text" name="recipient_name" id="recipient_name" class="form-control modern-input" value="{{ old('recipient_name', $otherDelivery->recipient_name) }}" required>
                        </div>

                        <div class="form-group">
                            <label for="recipient_phone" class="form-label">Phone</label>
                            <input type="text" name="recipient_phone" id="recipient_phone" class="form-control modern-input" value="{{ old('recipient_phone', $otherDelivery->recipient_phone) }}">
                        </div>

                        <div class="form-group">
                            <label for="recipient_address" class="form-label">Address <span class="text-danger">*</span></label>
                            <textarea name="recipient_address" id="recipient_address" class="form-control modern-input" rows="3" required>{{ old('recipient_address', $otherDelivery->recipient_address) }}</textarea>
                        </div>
            ...
            <div id="transport-info" class="collapse show">
                <div class="card-body modern-card-body">
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="text" name="vehicle_type" id="vehicle_type" class="form-control modern-input" value="{{ old('vehicle_type', $otherDelivery->vehicle_type) }}">
                            </div>
                            
                            <div class="form-group">
                                <label for="vehicle_number" class="form-label">Vehicle Number</label>
                                <input type="text" name="vehicle_number" id="vehicle_number" class="form-control modern-input" value="{{ old('vehicle_number', $otherDelivery->vehicle_number) }}">
                            </div>
                        </div>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="text" name="driver_name" id="driver_name" class="form-control modern-input" value="{{ old('driver_name', $otherDelivery->driver_name) }}">
                            </div>
                            
                            <div class="form-group">
                                <label for="driver_phone" class="form-label">Driver Phone</label>
                                <input type="text" name="driver_phone" id="driver_phone" class="form-control modern-input" value="{{ old('driver_phone', $otherDelivery->driver_phone) }}">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="notes" id="notes" class="form-control modern-input" rows="2">{{ old('notes', $otherDelivery->notes) }}</textarea>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="card-body modern-card-body">
                ...
                <div class="mobile-products-container" id="mobile-products">
                    @foreach($otherDelivery->items as $index => $item)
                        <div class="mobile-product-card product-row-mobile" data-row-index="{{ $index }}">
                                ...
                                <select name="product_id[]" class="form-control modern-select product-select-mobile select2-searchable" required>
                                    ...
                                    @foreach($products as $product)
                                        <option value="{{ $product->id }}" 
                                                data-stock="{{ $product->current_stock + $item->quantity }}" 
                                                data-box-pcs="{{ $product->category->box_pcs ?? 0 }}" 
                                                data-pieces-feet="{{ $product->category->pieces_feet ?? 0 }}"
                                                {{ $product->id == $item->product_id ? 'selected' : '' }}>
                                            {{ $product->name }} ({{ $product->current_stock + $item->quantity }} available)
                                        </option>
                                    @endforeach
                                </select>
                                <div class="stock-info">
                                    Available: <span class="available-stock-mobile">{{ $item->product->current_stock + $item->quantity }}</span>
                                </div>
                            </div>
                            
                            <div class="form-group">
                                <label class="form-label">Description</label>
                                <input type="text" name="description[]" class="form-control modern-input" value="{{ old('description.' . $index, $item->description) }}">
                            </div>
                            ...
                            <div class="quantity-controls">
                                ...
                                <div class="quantity-inputs">
                                    <div class="form-group quantity-group">
                                        ...
                                        <input type="number" name="quantity[]" class="form-control modern-input quantity-field-mobile" min="0.01" step="0.01" value="{{ old('quantity.' . $index, $item->quantity) }}" required>
                                        <input type="hidden" class="box-pcs-mobile" value="{{ $item->product->category->box_pcs ?? 0 }}">
                                        <input type="hidden" class="pieces-feet-mobile" value="{{ $item->product->category->pieces_feet ?? 0 }}">
                                    </div>
                                        ...
                                        <div class="row">
                                            <div class="col-6">
                                                <div class="form-group">
                                                    ...
                                                    <input type="number" name="cartons[]" class="form-control modern-input carton-field-mobile" min="0" value="{{ old('cartons.' . $index, $item->cartons) }}" readonly>
                                                </div>
                                            </div>
                                            <div class="col-6">
                                                <div class="form-group">
                                                    <label class="form-label">Pieces</label>
                                                    <input type="number" name="pieces[]" class="form-control modern-input pieces-field-mobile" min="0" value="{{ old('pieces.' . $index, $item->pieces) }}" readonly>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <tbody class="modern-tbody">
                                @foreach($otherDelivery->items as $index => $item)
                                    <tr class="product-row">
                                        ...
                                        <td class="available-stock text-center font-weight-bold">{{ $item->product->current_stock + $item->quantity }}</td>
                                        <td>
                                            <input type="number" name="quantity[]" class="form-control modern-input quantity-field" min="0.01" step="0.01" value="{{ old('quantity.' . $index, $item->quantity) }}" required>
                                        ...
                                        <td>
                                            <input type="number" name="cartons[]" class="form-control modern-input carton-field" min="0" value="{{ old('cartons.' . $index, $item->cartons) }}" readonly>
                                        </td>
                                        <td>
                                            <input type="number" name="pieces[]" class="form-control modern-input pieces-field" min="0" value="{{ old('pieces.' . $index, $item->pieces) }}" readonly>
                                        </td>
                                        <td>
        ...
        <div class="mobile-bottom-actions">
            <div class="mobile-button-group">
                ...
                <a href="{{ route('other-deliveries.show', $otherDelivery) }}" class="btn modern-btn modern-btn-outline btn-lg mobile-submit-btn">
        ...
        <div class="form-actions desktop-only">
            <div class="button-group">
                ...
                <a href="{{ route('other-deliveries.show', $otherDelivery) }}" class="btn modern-btn modern-btn-outline btn-lg">
                    <i class="fas fa-times"></i> Cancel
                </a>
                <a href="{{ route('other-deliveries.index') }}" class="btn modern-btn modern-btn-secondary btn-lg">
...
@section('additional_css')
    ...
    <style>
        /* Include all the same CSS from create view */
        /* Form styling */
        .form-label {
            font-size: 13px;
            font-weight: 600;
...
@section('additional_js')
    ...
    <script>
        $(document).ready(function() {
            ...
            // FIXED: Enhanced form validation and submission for edit
            $('#delivery-form').on('submit', function(e) {
                ...
                // Show loading state
                const submitBtn = $('#submit-btn, #submit-btn-desktop');
                const originalText = submitBtn.html();
                
                submitBtn.prop('disabled', true);
                submitBtn.html('<i class="fas fa-spinner fa-spin"></i> Updating...');
                
                console.log('Edit form validation passed, allowing natural submission');
            ...
            setTimeout(function() {
                ...
                $('.mobile-product-card').each(function() {
                    const card = $(this);
                    if (card.find('.quantity-field-mobile').val()) {
                        calculateCartonsAndPiecesMobile(card);
                    }
                });
            }, 500);
