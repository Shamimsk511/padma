...
@section('title', 'Edit Payable Transaction')

@section('page_title', 'Edit Payable Transaction')
...
@section('page_content')
    ...
    <form action="{{ route('payable-transactions.update', $transaction->id) }}" method="POST" id="payable-transaction-form">
        @csrf
        @method('PUT')
                                    ...
                                    <select name="payee_id" id="payee_id" class="form-control modern-select select2 @error('payee_id') is-invalid @enderror" required>
                                        ...
                                        @foreach($payees as $payee)
                                            <option value="{{ $payee->id }}"
                                                ...
                                                {{ old('payee_id', $transaction->payee_id) == $payee->id ? 'selected' : '' }}>
                        ...
                        <div class="info-section">
                            ...
                            <div class="section-content">
                                <div class="form-group">
                                    ...
                                    <input type="date" name="transaction_date" id="transaction_date" class="form-control modern-input @error('transaction_date') is-invalid @enderror" value="{{ old('transaction_date', $transaction->transaction_date->format('Y-m-d')) }}" required>
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="transaction_type" id="transaction_type" class="form-control modern-select @error('transaction_type') is-invalid @enderror" required>
                                        <option value="cash_in" @selected(old('transaction_type', $transaction->transaction_type) == 'cash_in')>Payment to Payee (Cash In)</option>
                                        <option value="cash_out" @selected(old('transaction_type', $transaction->transaction_type) == 'cash_out')>Received from Payee (Cash Out)</option>
                                    </select>
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="category" id="category" class="form-control modern-select @error('category') is-invalid @enderror" required>
                                        ...
                                        <optgroup label="Cash In Categories" id="cash-in-categories">
                                            <option value="payment" @selected(old('category', $transaction->category) == 'payment')>Payment</option>
                                            <option value="commission" @selected(old('category', $transaction->category) == 'commission')>Commission</option>
                                            <option value="adjustment" @selected(old('category', $transaction->category) == 'adjustment')>Adjustment</option>
                                            <option value="other_in" @selected(old('category', $transaction->category) == 'other_in')>Other</option>
                                        </optgroup>
                                        <optgroup label="Cash Out Categories" id="cash-out-categories">
                                            <option value="purchase" @selected(old('category', $transaction->category) == 'purchase')>Purchase</option>
                                            <option value="borrow" @selected(old('category', $transaction->category) == 'borrow')>Borrow</option>
                                            <option value="other_out" @selected(old('category', $transaction->category) == 'other_out')>Other</option>
                                        </optgroup>
                                        <optgroup label="Loan Categories" id="loan-categories">
                                            <option value="interest_payment" @selected(old('category', $transaction->category) == 'interest_payment')>Interest Payment</option>
                                        </optgroup>
                                    </select>
                                    @error('category')
                                        <div class="invalid-feedback">{{ $message }}</div>
                ...
                <div class="row">
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <div class="input-group">
                                ...
                                <input type="number" step="0.01" min="0.01" name="amount" id="amount" class="form-control modern-input @error('amount') is-invalid @enderror" value="{{ old('amount', $transaction->amount) }}" required>
                            </div>
                            @error('amount')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="col-md-4 loan-field">
                        <div class="form-group">
                            ...
                            <input type="number" step="0.01" min="0" name="principal_amount" id="principal_amount" class="form-control modern-input @error('principal_amount') is-invalid @enderror" value="{{ old('principal_amount', $transaction->principal_amount) }}">
                    ...
                    <div class="col-md-4 loan-field">
                        <div class="form-group">
                            ...
                            <input type="number" step="0.01" min="0" name="interest_amount" id="interest_amount" class="form-control modern-input @error('interest_amount') is-invalid @enderror" value="{{ old('interest_amount', $transaction->interest_amount) }}">
                    ...
                    <div class="col-md-4 loan-field" id="kisti-days-field">
                        <div class="form-group">
                            ...
                            <input type="number" min="0" name="kisti_days" id="kisti_days" class="form-control modern-input @error('kisti_days') is-invalid @enderror" value="{{ old('kisti_days', $transaction->kisti_days) }}">
                            @error('kisti_days')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
        ...
        <div class="card modern-card mt-4">
            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <select name="payment_method" id="payment_method" class="form-control modern-select @error('payment_method') is-invalid @enderror">
                                ...
                                <option value="bank_transfer" @selected(old('payment_method', $transaction->payment_method) == 'bank_transfer')>Bank Transfer</option>
                                <option value="credit_card" @selected(old('payment_method', $transaction->payment_method) == 'credit_card')>Credit Card</option>
                                <option value="check" @selected(old('payment_method', $transaction->payment_method) == 'check')>Cheque</option>
                                <option value="mobile_bank" @selected(old('payment_method', $transaction->payment_method) == 'mobile_bank')>Mobile Bank</option>
                                <option value="other" @selected(old('payment_method', $transaction->payment_method) == 'other')>Other</option>
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select name="account_id" id="account_id" class="form-control modern-select @error('account_id') is-invalid @enderror">
                                ...
                                @foreach($cashBankAccounts as $account)
                                    <option value="{{ $account->id }}" @selected(old('account_id', $transaction->account_id) == $account->id)>{{ $account->name }} [{{ $account->formatted_balance }}]</option>
                                @endforeach
                            </select>
                            @error('account_id')
                                <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <input type="text" name="reference_no" id="reference_no" class="form-control modern-input @error('reference_no') is-invalid @enderror" value="{{ old('reference_no', $transaction->reference_no) }}">
                            @error('reference_no')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                ...
                <div class="form-group">
                    ...
                    <textarea name="description" id="description" rows="3" class="form-control modern-input @error('description') is-invalid @enderror">{{ old('description', $transaction->description) }}</textarea>
                    @error('description')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
